...
raf-framework 是企业级微服务开发框架，封装中间件接入、异常处理、分布式追踪、安全加密，工具类等横切关注点，让业务开发者只需关注业务本身。
...
└── examples/                          # 示例项目（完整可运行）
    ...
    ├── raf-framework-datasource-starter/      # 多数据源路由（@DsSelector）
    ├── raf-framework-mybatis-starter/         # MyBatis-Plus + 分页
    ...
    ├── raf-framework-openapi-starter/         # OpenAPI 文档（Springdoc + Knife4j）
    └── raf-framework-kms-starter/             # 多云 KMS 密钥管理与字段加密
```

## 示例项目

`examples/` 下每个 starter 对应一个独立示例，需要多服务协作的场景（如 Dubbo）保留多模块结构。

- `raf-example-web-starter` - Web 基础（统一响应、异常处理、日志）
- `raf-example-mybatis-starter` - MyBatis-Plus 多数据源 + 读写分离
- `raf-example-redis-starter` - Redis + Redisson 分布式锁
- `raf-example-dubbo-starter` - Dubbo RPC（多模块：api / provider / consumer）
- `raf-example-rabbit-starter` - RabbitMQ
- `raf-example-rocketmq-starter` - RocketMQ（含事务消息）
- `raf-example-kafka-starter` - Kafka
- `raf-example-elasticsearch-starter` - Elasticsearch
- `raf-example-mongodb-starter` - MongoDB 多数据源
- `raf-example-shardingsphere-starter` - ShardingSphere 分库分表
- `raf-example-gateway-starter` - Spring Cloud Gateway
- `raf-example-nacos-starter` - Nacos 配置中心 & 服务发现
- `raf-example-okhttp-starter` - OkHttp 第三方 API 调用
- `raf-example-openapi-starter` - OpenAPI 文档（Springdoc + Knife4j）
- `raf-example-monitor-starter` - Prometheus 监控
- `raf-example-sentry-starter` - Sentry 错误追踪
- `raf-example-kms-starter` - 多云 KMS 密钥管理与字段加密
...
- `DynamicDataSourceAutoConfiguration`, `MybatisPlusExtensionAutoConfiguration` - 数据库层
...
- `OkHttpAutoConfig` - HTTP 客户端
- `ElasticsearchAutoConfiguration`, `ShardingSphereAutoConfiguration` - 扩展组件
- `KmsAutoConfiguration`, `SentryAutoConfiguration` - 安全与可观测性

所有配置使用条件注解（`@ConditionalOnProperty`、`@ConditionalOnClass`）支持热插拔。

## 异常处理体系

框架定义四层异常层次（`com.raf.framework.core.common.exception`）：
...
所有 API 响应使用 `RafResult<T>`，格式为 `{"code": 0, "msg": "success", "data": {}}`。响应码：0 成功 | 10401 未授权 | 10403 权限不足 | 10404 未找到 | 10405 HTTP方法错误 | 10429 限流 | 10500 服务错误 | 10700 参数错误 | 10800 第三方接口错误 | 10000+ 业务异常（应用自定义）

所有组件默认关闭，通过 `raf.{component}.enabled=true` 启用：

- `raf.datasource.{name}` - 多数据源（`@DsSelector` 路由），`raf.mybatis.pagehelper` 分页
- `raf.redis` / `raf.redisson` - Redis + 分布式锁，`raf.redis.custom-cache.{name}` 自定义缓存 TTL
