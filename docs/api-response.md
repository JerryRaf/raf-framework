...
| 字段        | 类型                    | 必填 | 说明                       |
|-----------|-----------------------|----|--------------------------|
| `code`    | String                | Y  | `"0"` 表示成功               |
| `message` | String                | Y  | 面向用户的提示信息；前端可展示或忽略       |
...
| 字段        | 类型     | 必填 | 说明                    |
|-----------|--------|----|-----------------------|
| `code`    | String | Y  | 5 位业务错误码              |
| `message` | String | Y  | 面向用户的错误文案；前端可直接 Toast |
...
- `XX`（前 2 位）— **服务 ID**，统一分配。
- `YYY`（后 3 位）— **错误流水号**。
    - 最佳实践：后 3 位尽量与对应的 HTTP 状态码对齐（如 `400` 表示参数错误，`409` 表示冲突，`500` 表示内部错误）。
...
| BFF聚合层        | `17`      | `17001-17999` | BFF 聚合 |
| 用户            | `21`      | `21000-21999` | 用户     |
| 交易服务          | `25`      | `25000-25999` | 交易     |
| 运营后台服务        | `81`      | `81000-81999` | 订单     |
