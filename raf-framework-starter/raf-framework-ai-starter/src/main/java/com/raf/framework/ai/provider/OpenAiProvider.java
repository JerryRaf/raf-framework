    ...
    public Flux<String> streamChat(AiRequest request, List<Message> history) {
        try {
            return chatModel.stream(buildPrompt(request, history))
                ...
                .onErrorMap(e -> new InfrastructureException("OpenAI stream failed: " + e.getMessage(), e));
        } catch (Exception e) {
            return Flux.error(new InfrastructureException("OpenAI stream failed: " + e.getMessage(), e));
        }
    }
