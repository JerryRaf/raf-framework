...
import com.raf.framework.core.util.IpUtil;
...
public class AccessJsonLogBuilder {
    ...
    public AccessJsonLogBuilder(JsonService json, AuditProperties auditProperties) {
    ...
    public static AccessJsonLogBuilder accessJsonLogBuilder(JsonService json, AuditProperties auditProperties) {
    ...
    public AccessJsonLogBuilder put(HttpServletRequest request) {
        ...
        if (level >= AuditProperties.LogLevel.REQ_HEADERS.getLevel()) {
            this.put("Q_IP", IpUtil.getIpAddr(request)).put("Q_HEADERS", AuditLogUtil.getAllHeaders(request));
        }
        return this;
    }
    ...
    public AccessJsonLogBuilder put(HttpServletResponse response) {
        // Always record HTTP status code and it's available at BASIC level
        this.put("R_STATUS", response.getStatus());
        if (auditProperties.getLog().level.getLevel() >= AuditProperties.LogLevel.RSP_HEADERS.getLevel()) {
            return this.put("R_HEADERS", AuditLogUtil.getAllHeaders(response)).put("R_CODE", response.getStatus());
        }
        return this;
    }
                        ...
                        if (responsePayLoad.startsWith(startStr) && responsePayLoad.endsWith(endStr)) {
                            try {
                                Map<String, Object> map =
                                        json.parse(responsePayLoad, new TypeReference<Map<String, Object>>() {
                                        });
                                ...
                                this.put(
                                        "R_CODE",
                                        String.valueOf(RafResult.success().getCode()));
                            }
                        } else {
                            this.put(
                                    "R_CODE", String.valueOf(RafResult.success().getCode()));
                        }
                    });
        } catch (Exception ex) {
